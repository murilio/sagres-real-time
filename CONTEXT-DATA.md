...
App Next.js único (`app/`) com Prisma embutido, decisão de pivot já commitada. Rota `/` mostra tela inicial do painel: cabeçalho + mapa Leaflet (`react-leaflet`) enquadrando a Paraíba via `bounds`, ocupando `100dvh`, com contorno do estado e dos 223 municípios desenhado a partir de `app/public/geo/paraiba-municipios.geojson` (224 features, malha do IBGE casada por nome contra a semente `municipios-tce-pb.csv`). Os 223 municípios têm preenchimento azul claro (`#93c5fd`, opacidade 0.45) em repouso, escurecendo (`#2563eb`, opacidade 0.7) e mostrando tooltip com o nome no hover, via `onEachFeature` em `mapa-paraiba.tsx`; o contorno do estado segue sem preenchimento. Verificado por execução: `pnpm typecheck` limpo, `pnpm build` ok, teste visual no navegador (zoom/pan ok, Paraíba enquadrada, contornos desenhados); hover confirmado disparando `mouseover` manualmente via JS no DOM.
...
Route Handler de ingestão não existe ainda — só orientação de desenho em `docs/desenho-ingestao.md`. Plataforma de deploy definida: Vercel, projeto `sagres-real-time` (owner `murilios-projects`), Root Directory `app`, banco Postgres no Supabase (ref `vcwamntedeqdazvsxutn`) via pooler pgbouncer (`DATABASE_URL`, porta 6543) + conexão direta pro CLI (`DIRECT_URL`, porta 5432). Deploy funcionando por relato do usuário (não verificado por comando nesta sessão) após corrigir Framework Preset para Next.js e Node 22.x na dashboard Vercel.
Mudanças dos contornos **não commitadas**: diff em `app/package.json`, `app/pnpm-lock.yaml`, `app/src/components/mapa-paraiba.tsx`, mais untracked `app/public/geo/`, `app/src/scripts/`.
...
- [2026-09-12] Plataforma de deploy: Vercel — substitui a decisão anterior de deixar `A DEFINIR`; projeto `sagres-real-time` linkado com Root Directory `app`.
- [2026-09-12] Banco de produção/teste: Postgres no Supabase (pooler pgbouncer pra runtime, conexão direta pro CLI) — não usa o client `supabase-js`, só a connection string.
...
- [2026-09-08] Malha territorial do IBGE em vez do Nominatim para os contornos do mapa — pedido explícito do usuário, decisão trocada após teste real: Nominatim devolveu 2 resultados para "Bananeiras" (município vs. distrito interno) e um único município sem simplificação já pesava 43 KB; IBGE devolve os 223 municípios completos numa só chamada, sem ambiguidade, 215 KB no total (`qualidade=intermediaria`).
- [2026-09-08] Casar município do IBGE com a semente TCE-PB por nome normalizado, não por código IBGE — o projeto não guarda `codigo_ibge` (lacuna já registrada na ADR-0002); verificado antes de implementar que os 223 nomes batem 1:1 sem colisão.

## Demandas

- [2026-09-12] Configurar deploy do app na Vercel → projeto `sagres-real-time` linkado via `vercel link` (Root Directory `app`), banco Supabase configurado (`DATABASE_URL` pooler + `DIRECT_URL` direta), corrigido `prisma.config.ts` pra ler `DIRECT_URL` em vez de `DATABASE_URL` (pooler em modo transaction não suporta os advisory locks/DDL do `migrate`), corrigido Framework Preset do projeto Vercel de `Other` pra `Next.js` e Node Version pra 22.x via dashboard (causa do 404 em todas as rotas apesar do build "Completed"); deploy funcionando confirmado por relato do usuário, não por comando desta sessão → `app/prisma.config.ts`, `.gitignore`
- [2026-09-08] Pintar cada município com cor clarinha e escurecer no hover com tooltip do nome, "quero que pinte cada cidade com uma cor clarinha e ao passar o mouse a cor escuresça e aparece um tooltip com o nome da cidade" → `mapa-paraiba.tsx` ganhou `fillColor`/`fillOpacity` em repouso e no hover (via `onEachFeature`/`layer.on("mouseover"/"mouseout")`) mais `layer.bindTooltip` com o nome; só os 223 municípios, contorno do estado sem preenchimento; verificado por execução: `pnpm typecheck` e `pnpm build` limpos, hover confirmado disparando `mouseover` manualmente via JS no DOM (simulação de mouse da automação de browser não gera evento nativo sobre SVG do Leaflet); `documenter` despachado em paralelo para `docs/referencia/tela-mapa-paraiba.md`, resultado ainda não visto por este registro; nada commitado → `app/src/components/mapa-paraiba.tsx`
- [2026-09-08] Adicionar contorno da Paraíba e dos 223 municípios ao mapa, "baixar coordenadas via Nominatim" trocado por malha do IBGE após teste real de ambiguidade → `app/src/scripts/baixar-contornos-municipios.ts` (script de execução única, `pnpm geo:baixar`) casa os 223 municípios do IBGE com `municipios-tce-pb.csv` por nome normalizado e gera `app/public/geo/paraiba-municipios.geojson` (224 features, cada uma com `codigo_tce`); `mapa-paraiba.tsx` busca o GeoJSON client-side e desenha `<GeoJSON>` do react-leaflet (estado em azul grosso, municípios em cinza fino, ambos sem preenchimento) mais atribuição do IBGE; `@types/geojson@7946.0.16` fixado; verificado por execução: 223/223 casaram sem falha, `pnpm typecheck` e `pnpm build` limpos, teste visual confirmou os contornos; `documenter` despachado em paralelo para `docs/`, resultado ainda não visto por este registro; nada commitado → `app/src/scripts/baixar-contornos-municipios.ts`, `app/public/geo/paraiba-municipios.geojson`, `app/src/components/mapa-paraiba.tsx`, `app/package.json`, `app/pnpm-lock.yaml`
...
- [ ] Definir cron externo pra chamar a rota de ingestão na Vercel (scheduler da plataforma vs. cron de sistema apontando pra URL)
...
- [ ] Mapa coroplético (colorir município por indicador de fiscalização) — intenção registrada em comentário de código em `mapa-paraiba.tsx`, não decisão implementada; depende de haver indício/regra de fiscalização por município, que ainda não existe

## Becos sem saída

- Testar hover do mapa clicando/movendo o mouse via automação de browser sobre elementos SVG do Leaflet — a simulação não gera evento nativo `mouseover`; captura de tela não mostra o efeito. Confirmado disparando o evento manualmente via JS no DOM.
...
- Colar `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` no `.env` — projeto não usa o client `supabase-js`, conecta via Prisma + `pg`/`@prisma/adapter-pg` direto na connection string do Postgres. Corrigido substituindo pelas URLs de conexão corretas (`DATABASE_URL`/`DIRECT_URL`).
