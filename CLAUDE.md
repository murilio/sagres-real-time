...
`.claude/agents/` holds seven project-scoped agents, versioned with the code. They are written to be domain-agnostic — nothing in them references SAGRES — so they can be copied to `~/.claude/agents/` or packaged as a plugin once they prove out here.

- **`orchestrator`** — takes a raw demand, decomposes it into capabilities and routes it to the specialist that declares each one. It has no hardcoded roster: it reads `.claude/agents/*.md` frontmatter at run time, so it stays correct as agents are added or removed. Default mode is PLANO — it returns the routing plan and stops; it only dispatches when told to execute or when the whole flow is read-only. Nested dispatch is verified to work in this harness (a subagent can spawn a subagent), but depth is capped at 1 and irreversible or outward-facing steps (commit, push, deploy, migration, deletion) are never dispatched without explicit authorization in the incoming request. Ordering rule it enforces: investigate → design → implement → audit → version → document/record state; parallel only between read-only agents with no result dependency.
