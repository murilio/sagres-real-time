...
`.claude/agents/` holds five project-scoped agents, versioned with the code. They are written to be domain-agnostic — nothing in them references SAGRES — so they can be copied to `~/.claude/agents/` or packaged as a plugin once they prove out here.
